...
class HierSGNN(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.sequential_type = args.sequential_type if experimental else None
        self.sublevel_graph_results = []
    ...
    def hierarchical_successive_train_net(self, input_dict,
        ...
        og_data = self.graphs[self.graph_level].copy() if self.sequential_type == "sequential_lift" else  0
        # data = data.to(device)
        ...
        if epoch in self.graph_lift_intervals and epoch != total_epochs:#% (total_epochs // self.data_size) == 0 and epoch != 0:
            ...
            self.sublevel_graph_results.append((val_acc, val_roc))
            ...
            with torch.no_grad():
                for batch_size, n_id, adjs in self.graphlevelloader:#batch in self.graphlevelloader:
                    ...
                    for nid in n_id.detach().cpu().numpy():
                        """ changed here """
                        data = data if self.sequential_type == "successive_init" else og_data
...
"""
                    ...
                    Begin Ablation Study and Experimental Implimentations
